...
class FenceLeafletLayer():
    ...
    def __init__(self, name, fence, cube_name):
        ...
        self.vmin = vmin
        self.vmax = vmax
        self.bounds = [[hmin, vmax], [hmax, vmin]]
        self.center = [(hmin+hmax)/2, (vmin+vmax)/2]
        self.z_arr = values
    ...
    def get_h_layer(self, fn):
        ...
        y = values[0]
        x = values[1]
        data = [[v[0], self.vmax-v[1]+self.vmin] for v in values]
        return {
                    'name': 'surface',
                    'checked': True,
                    'base_layer': False,
...
class SeismicFence():
    ...
    def __init__(self, app):
        ...
        self.surface = './example-data/TopReek--depth.gri'
        self.seismic = './example-data/seismic--synt--2001.segy'
    ...
    def set_callbacks(self, app):
        ...
        def change_map(coords):
            ...
            df = pd.DataFrame(data)
            df.columns = df.columns.astype(str)
            cube = xtgeo.cube.Cube(self.seismic)
            s = FenceLeafletLayer('seismic', df, self.seismic)
            h = s.get_h_layer(self.surface)
